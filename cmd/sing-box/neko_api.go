...
func BoxTest(Mode C.int, Address *C.char, Url *C.char, Timeout C.int, SpeedUrl *C.char, SpeedTimeout C.int, CoreConfig *C.char) *C.char {
	...
	var results []string
	testResult := func(v any, err error) {
		if err != nil {
			results = append(results, err.Error())
		} else {
			switch val := v.(type) {
			case int64:
				results = append(results, strconv.FormatInt(val, 10))
			case float64:
				results = append(results, fmt.Sprintf("%.2f", val))
			default:
	...
	if mode&UdpTest != 0 {
		...
		if err == nil {
			...
			if _, err = pc.Write(dnsQuery); err == nil {
				var buf [512]byte
				_, err = pc.Read(buf[:])
			}
		}
		testResult(time.Since(start).Milliseconds(), err)
	}
	...
	if mode&SpeedTest != 0 {
		...
		testResult(float64(n)/duration/MiB, err)
	}

	if mode&IpTest != 0 {
		var inIP, outIP, country string
		...
		results = append(results, inIP+" → "+outIP, country)
	}
